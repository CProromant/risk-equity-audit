...
- [x] **`group_capture(scores, need, groups, k, weights)`** — captura por subgrupo con IC de dominio: el eje de equidad ("¿de *quién* es la necesidad omitida?"). Test con grupos alineado/invertido de valor conocido + singleton.
- [x] **`label_blend_frontier(scores_a, scores_b, need, k, weights, groups, alphas)`** — barre α·A+(1−α)·B; la frontera de trade-off. Test: α=0 y α=1 recuperan `top_k_capture` de cada etiqueta; shares por grupo suman 1.
